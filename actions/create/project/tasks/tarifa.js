...
function initBuilder(response) {
    return builder.init(pathHelper.resolve(response.path), response.options.verbose).then(function () {
        return response;
    }).fail(function (error) {
...
function log(response) {
    if (response.options.verbose)
        print.success('project folders created %s', pathHelper.resolve(response.path));
...
function createDotTarifaFile(response) {
    ...
    return fs.write(pathHelper.resolve(response.path, '.tarifa.json'), JSON.stringify(o, null, 2))
        .then(function () { return response; });
}
...
function createUserCheckScripts(response) {
    var content = "module.exports = function (msg) {\n    return msg;\n}",
        write = function (platform) {
            return fs.write(pathHelper.resolve(response.path, 'project/bin', format('check_%s.js', platform)), content);
        };
