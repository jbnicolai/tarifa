...
function isColor(str) {
    if (typeof str !== 'string') return false;
    return colorHelper.validate(str);
};
...
function isProvisioningProfilePath(str) {
    if (typeof str !== 'string') return false;
    return !fs.existsSync(path.resolve(str));
};
...
isProjectId.error               = "must be a valid Java package name that does not contain any hyphen (-) or underscore (_)"
isProjectPath.error             = "must be a non-existing or empty folder";
isProvisioningProfilePath.error = "must be a non-existing file";
...
module.exports = {
    ...
    isProvisioningProfilePath: isProvisioningProfilePath,
    isUrl:                     isUrl,
