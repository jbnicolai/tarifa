...
function getActivityName(file, name) {
    return parse(file).then(function (xml) {
        return xml.manifest.application[0].activity[0].$['android:name'];
    });
}

function setActivityName(file, name) {
    return parse(file).then(function (xml) {
...
module.exports.setActivityName = setActivityName;
