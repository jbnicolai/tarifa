...
function parseResponse(response) {
    ...
    if(response.certificate_path) {
        o.configurations.wp8.prod.certificate_path = response.certificate_path;
    }
    ...
    settings.configurations.forEach(function (conf) {
        ...
        if(platform === 'wp8') {
            ...
            if(conf === 'stage' && obj.deploy)
                o[conf].certificate_path = obj.wp8_certificate_path;
        }
    });
    return o;
}
...
tarifaFile.getPlatformConfigs = function (parsedSettings, platform) {
    var confs = [];
    for (var conf in parsedSettings.configurations[platform]) {
        confs.push(conf);
    }
    return confs;
};
