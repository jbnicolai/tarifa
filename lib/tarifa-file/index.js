...
function isPrivateKey(privateKeys) {
    return function(key) {
        return (privateKeys[key] !== undefined) || isPrivateKeyByDefault(key);
    };
}
...
function write(dirname, obj) {
    ...
    var publicPath = path.join(dirname, settings.publicTarifaFileName),
        ...
        parsedPrivateKeys = obj[tarifaFile.parse.parsedPrivateKeys()] || {},
        ...
        publicObj = collections.toMultiLevelObject(collections.filterKeys(oneLvlObj, function (key) {
            return !isPrivateKey(parsedPrivateKeys)(key);
        })),
        privateObj = collections.toMultiLevelObject(collections.filterKeys(oneLvlObj, isPrivateKey(parsedPrivateKeys)));
    return Q.all([
