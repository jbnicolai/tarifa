var Q = require('q'),
    ...
    installedPlatforms = require('../cordova/platforms').installedPlatforms,
...
tarifaFile.checkPlatforms = function (platforms) {
    var diff = difference(platforms, settings.platforms),
        l = diff.length,
        errSupportMsg = 'Following platform%s %s not supported: %s',
        errInstallMsg = 'Following platform%s %s not installed on host: %s';
    if(l) return Q.reject(util.format(errSupportMsg, l>1 ? 's' : '', l>1 ? 'are' : 'is', diff.join(', ')));

    return installedPlatforms().then(function (plts) {
        var d = difference(platforms, plts.map(function (p) {
                return p.value;
            })),
            len = d.length;
        if(len)
            return Q.reject(util.format(errInstallMsg, len>1 ? 's' : '', len>1 ? 'are' : 'is', d.join(', ')));
        else
            return platforms;
    });
};
