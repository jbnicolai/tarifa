...
function checkSigningLabel(platform, config, attributes, filePaths) {
    return function (o) {
        if(config && o.configurations[platform][config]) {
            var confObj =  o.configurations[platform][config];
            if(confObj.sign) {
                var label = confObj.sign;
                if(o.signing[platform][label]){
                ...
                return Q.reject(format("signing label `%s` is not defined in `signing` section", label));
            }
            return o;
        }
        return o;
...
function checkSigningIOS(platform, config) {
    if(platform === 'ios') {
        return checkSigningLabel(
            platform,
            config,
            ['identity', 'provisioning_path', 'provisioning_name'],
            ['provisioning_path']
        );
    } else {
...
function checkSigningAndroid(platform, config) {
    if(platform === 'android') {
        return checkSigningLabel(
            platform,
            config,
...
function checkSigningWp8(platform, config) {
    if(platform === 'wp8') {
        return checkSigningLabel(
            platform,
            config,
...
module.exports = function (dirname, platform, config) {
    ...
    .then(checkSigningAndroid(platform, config))
    .then(checkSigningIOS(platform, config))
    .then(checkSigningWp8(platform, config));
};
