...
function copyAssets(mapping, platform, configuration, type) {
    ...
    return tarifaFile.parse(cwd).then(function (localSettings) {
        return mapping[platform].reduce(function (promise, tuple) {
            ...
            imgPath, src, relative_path, dest;
            ...
            // img path potentially overriden in tarifa.json
            if(confSettings.assets_path) imgPath = confSettings.assets_path;

            // reject if imgPath directory doesn't exist
            if(!fs.existsSync(imgPath)) return Q.reject(imgPath + ' directory does not exist');

            src =  path.join(cwd, imgPath, type, tuple.src);
