...
function generateFromFile(mapping, file, type, root, platforms, config, verbose) {
    ...
    return checkImageSize(file).then(function(size) {
       size = size.split('x')[0];
       if (size < 192)
           return Q.reject('The provided image is too small. It must be a square of at least 192px side.');
    }).then(function () {
...
function createFolders(root, platforms, configuration, withSplashscreens) {
    var foldersPromises = [];
