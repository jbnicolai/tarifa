...
function androidId(string) {
    return /([\p{L}_$][\p{L}\p{N}_$]*\.)*[\p{L}_$][\p{L}\p{N}_$]*/.test(string);
}
    ...
    .then(function(obj) {
        ...
        if (platform === 'android') {
            if (!androidAppLabel(merged.app_label))
                throw new Error(formatError('[app_label] must be a string'));
            if (!androidName(merged.name))
                throw new Error(formatError('[name] must be a valid java class name'));
            if (!androidId(merged.id))
                throw new Error(formatError('[id] must be a valid java namespace'));
        }
