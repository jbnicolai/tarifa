...
function write(dirname, obj) {
    var publicPath = path.join(dirname, settings.publicTarifaFileName),
        ...
        flatObj = collections.flatten(obj),
        publicObj = collections.unflatten(collections.filterKeys(flatObj, function (key) {
            return !isPrivateKey(key);
        })),
        privateObj = collections.unflatten(collections.filterKeys(flatObj, isPrivateKey));
