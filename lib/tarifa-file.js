...
function parseResponse(response) {
    ...
    if(response.deploy) {
        o.deploy = {
            ...
            hockeyapp_user: "put your hockeyapp user here",
...
tarifaFile.parseConfig = function (filePath, platform, config) {
    ...
    .then(function(obj) {
        ...
        if (platform) {
            ...
            if(platform === 'android' && !validator.isJavaPackageName(merged.id))
                return Q.reject(new Error(formatError('[' + platformPath + '.id] ' + validator.isJavaPackageName.error)));
