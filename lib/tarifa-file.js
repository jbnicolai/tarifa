...
function parseResponse(response) {
    ...
    response.platforms.forEach(function (platform) {
        ...
        if(platform === 'wp8') {
            o.configurations[platform]['default'].guid = uuid.v4();
        }

        settings.configurations.forEach(function (conf) {
            o.configurations[platform][conf] = {
                id : response.id,
                product_name : response.name + ' ' + conf,
                product_file_name : response.name.replace(/ /g, '-')
            };
            if(platform === 'wp8') {
                o.configurations[platform][conf].guid = uuid.v4();
                if(conf === 'production' || conf === 'staging')
                    o.configurations[platform][conf].release_mode = true;
                if(conf === 'staging' && response.deploy)
                    o.configurations[platform][conf].sign_mode = true;
