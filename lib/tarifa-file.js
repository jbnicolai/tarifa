    ...
    .then(function(obj) {
        ...
        if (platform) {
            ...
            if (!validProductName(merged.product_name))
                return Q.reject(new Error(formatError('[' + platformPath + '.product_name] must be a string')));
            if (!validProductFileName(merged.product_file_name))
                return Q.reject(new Error(formatError('[' + platformPath + '.product_file_name] must be a valid ascii string')));

            if(platform === 'android' && !validIdForAJavaPackage(merged.id))
                return Q.reject(new Error(formatError('[' + platformPath + '.id] must be a valid java class name')));
            if(platform === 'ios' && !validIdForABundleId(merged.id))
                return Q.reject(new Error(formatError('[' + platformPath + '.id] must be a string that contains only alphanumeric characters (A-Z,a-z,0-9), hyphen (-), and period (.)')));
        }
